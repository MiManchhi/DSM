...
{
    //隶属组
    if(strlen(cfg_gpname) > ENCRYPT_GROUPNAME_MAX)
        logger_fatal("groupname too big %lu > %d", strlen(cfg_gpname), ENCRYPT_GROUPNAME_MAX);
    // 绑定端口号
    if(cfg_bindport <= 0)
    ...
    // 打印配置信息
	logger("cfg_gpname: %s, cfg_taddrs: %s, "
		"cfg_maddrs: %s, cfg_raddrs: %s, "
		"cfg_bindport: %d, cfg_interval: %d, cfg_mtimeout: %d, "
		"cfg_maxconns: %d, cfg_ctimeout: %d, cfg_rtimeout: %d, "
		"cfg_ktimeout: %d",
		cfg_gpname, cfg_taddrs,
		cfg_maddrs, cfg_raddrs,
