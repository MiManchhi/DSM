...
int encrypto_c::client(char const *requ, long long resqulen, char *&key, long long &keylen) const
{
    ...
    for (int i = 0; i < nids; ++i)
    {
        ...
        {
            logger("connect id success, addr:%s", g_eaddrs[nrand].c_str());
            break;
        }
        else
        {
            logger("connect id fail, addr:%s", g_eaddrs[nrand].c_str());
            nrand = (nrand + 1) % nids;
        }
    }
    if(!conn.alive()) //没有连接成功
    {
        logger_error("connect id fail, addrs:%s", cfg_eaddrs);
        return -1;
    }
    //向ID服务器发送请求
