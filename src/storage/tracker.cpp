...
int tracker_c::join(acl::socket_stream *conn) const
{
    ...
    request[BODYLEN_SIZE] = CMD_TRACKER_JOIN;
    request[BODYLEN_SIZE + COMMAND_SIZE] = 0;
    storage_join_body_t *sjb = reinterpret_cast<storage_join_body_t *>(request + HEADLEN);
    // 版本--组名--主机名--端口号--启动时间--加入时间
    strcpy(sjb->sjb_version, g_version);
    strcpy(sjb->sjb_groupname, cfg_gpname);
    strcpy(sjb->sjb_hostname, g_hostname.c_str());
    ston(cfg_bindport, sjb->sjb_port);
    lton(g_stime, sjb->sjb_stime);
    lton(time(NULL), sjb->sjb_jtime);
    // 发送请求
...
int tracker_c::beat(acl::socket_stream *conn) const
{
    ...
    char request[requestlen] = {};
    llton(bodylen, request);
    request[BODYLEN_SIZE] = CMD_TRACKER_BEAT;
    request[BODYLEN_SIZE + COMMAND_SIZE] = 0;
    storage_beat_body_t *sbb = reinterpret_cast<storage_beat_body_t *>(request + HEADLEN);
    //组名--主机名
    strcpy(sbb->sbb_groupname, cfg_gpname);
    strcpy(sbb->sbb_hostname, g_hostname.c_str());
    // 发送请求
    if(conn->write(request,requestlen) < 0)
    {
        logger_error("write fail:%s,requestlen:%lld,to:%s", acl::last_serror(), requestlen, conn->get_peer());
