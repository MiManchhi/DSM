...
int cache_c::set(char const *key, char const *value, int timeout) const
{
    ...
    if(!g_rconns)
    {
        logger_warn("redis connection pool is null, key:%s, value:%s, timeout:%d",storage_key.c_str(),value,timeout);
        return ERROR;
    }
    //从Redis连接池中拿出一个连接
    acl::redis_client* rconn = (acl::redis_client*)g_rconns->peek();
    if(!rconn)
    {
        logger_warn("peek redis connection fail, key:%s, value:%s, timeout:%d",storage_key.c_str(),value,timeout);
        return ERROR;
    }
    //构造Redis客户端
    acl::redis redis;
    ...
    logger("set key_value_pair ok ,key:%s, value:%s, timeout:%d",storage_key.c_str(),value,timeout);
